...
km_mp3.cpp - Implementation of the Arduino software serial for ESP32.
...
unsigned char mp3_voice_table[30]={0x34,0x35,0x33,0x36,0x33,0x37,0x33,0x38,0x33,0x39,0x34,0x30,0x34,0x31,0x34,0x32,0x34,0x33,0x34,0x34,0x33,0x33,0x33,0x34,0x33,0x35,0x33,0x30,0x31,0x36};

// As the Arduino attachInterrupt has no parameter, lists of objects
// and callbacks corresponding to each possible GPIO pins have to be defined
km_mp3 *ObjList[MAX_PIN + 1];
...
bool km_mp3::isValidGPIOpin(uint8_t pin)
{
  if (pin < MAX_PIN)
  {
...
void km_mp3::mp3_volume(uint8_t level){
...
void km_mp3::mp3_play_chapter(uint16_t chapter){
...
void km_mp3::mp3_loop_single_play(){
...
void km_mp3::mp3_playDistance(float s){

    int num = s*10;

    unsigned char Bit1 = num/1000;

    if (Bit1 > 9)return;

    num = num % 1000;unsigned char Bit2 = num/100;num = num % 100;unsigned char Bit3 = num/10;num = num % 10;unsigned char Bit4 = num;

    unsigned char play[20];
    ...
    if(Bit1){
        ...
        play[index++]=mp3_voice_table[Bit1*2];

        play[index++]=mp3_voice_table[Bit1*2+1];

      play[index++]=mp3_voice_table[22];

      play[index++]=mp3_voice_table[23];

    }

    if(Bit2){

      play[index++]=mp3_voice_table[Bit2*2];

      play[index++]=mp3_voice_table[Bit2*2+1];

      play[index++]=mp3_voice_table[24];

      play[index++]=mp3_voice_table[25];

    }

    if(!((Bit3 == 0) && (Bit2 == 0) && (Bit1 > 0))){

        play[index++]=mp3_voice_table[Bit3*2];

        play[index++]=mp3_voice_table[Bit3*2+1];

    }

    if(Bit4){

      play[index++]=mp3_voice_table[26];

      play[index++]=mp3_voice_table[27];

       play[index++]=mp3_voice_table[Bit4*2];

       play[index++]=mp3_voice_table[Bit4*2+1];

    }

   play[index++]=mp3_voice_table[28];

   play[index++]=mp3_voice_table[29];

    play[2]=index-3;

    play[index++]=mp3_SumCheck(play,play[2]+4-1);

    write(play,play[2]+4);

}

unsigned char km_mp3::mp3_SumCheck(unsigned char *buf, unsigned char len){
...
km_mp3::km_mp3(int receivePin, int transmitPin, bool inverse_logic, unsigned int buffSize)
{
...
void km_mp3::begin(uint32_t speed)
{
...
long km_mp3::baudRate()
{
  return ESP.getCpuFreqMHz() * 1000000 / m_bitTime;
}

void km_mp3::setTransmitEnablePin(int transmitEnablePin)
{
